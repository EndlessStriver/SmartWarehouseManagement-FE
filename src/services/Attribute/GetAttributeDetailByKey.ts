...
const GetAttributeDetailByKey = async (data: GetAttributeDetailProps, navigate: NavigateFunction): Promise<GetAttributeDetailResponse | undefined> => {
    ...
    try {
        ...
        if (!token) {
            ...
            const response = await axios.get(`${HOST}/${returnNameAttribute(data.id)}/name-pagination?limit=${data?.limit || 10}&offset=${data?.offset || 1}&order=${data?.order || Order.ASC}&name=${data.key || ""}`, {
                headers: {
...
export default GetAttributeDetailByKey;
